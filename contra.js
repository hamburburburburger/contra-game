...
class Enemy {
  constructor(x, y, type) {
    ...
    this.bossShootTimers = []; // clean up timeouts
  ...
  updateBoss(dt, player, dist) {
    ...
    if (this.shootCooldown <= 0) {
      ...
      if (pattern === 0) {
        ...
        for (let i = -3; i <= 3; i++) {
          const angle = i * 0.2;
          bullets.push(createEnemyBullet(
            this.x + this.w/2, this.y + 10,
            Math.cos(angle) * 4 * Math.sign(dx),
            Math.sin(angle) * 4,
            '#ff44ff', 5
          ));
        }
      } else if (pattern === 1) {
        // Aimed shot with timers
  ...
  shootAt(player) {
    ...
    const ny = dy/dist;
    bullets.push(createEnemyBullet(
      this.x + this.w/2, this.y + (this.type === 'shooter' ? 14 : 7),
      nx * 4, ny * 4 - 1,
      '#ff4444', 4
    ));
  }
  ...
  takeDamage(amount) {
    ...
    if (this.hp <= 0) {
      ...
      for (const tid of this.bossShootTimers) clearTimeout(tid);
    }
  }
}

function createEnemyBullet(x, y, vx, vy, color, size) {
  return {
    x, y, vx, vy,
    weapon: { color, speed: 4, damage: 1, size, fireRate: 0 },
...
const game = {
  ...
  buildLevel() {
    const groundSegments = [
      { x: 0, w: 600 }, { x: 700, w: 400 }, { x: 1200, w: 300 },
      { x: 1600, w: 500 }, { x: 2200, w: 300 }, { x: 2600, w: 400 },
      { x: 3100, w: 600 }, { x: 3800, w: 300 }, { x: 4200, w: 500 },
      { x: 4800, w: 400 }, { x: 5300, w: 600 }, { x: 6000, w: 400 },
    ];
  ...
  spawnEnemies() {
    const enemyDefs = [
      // Guards
      { x: 400, type: 'guard' }, { x: 650, type: 'guard' },
      { x: 900, type: 'guard' }, { x: 1150, type: 'guard' },
      { x: 1450, type: 'guard' }, { x: 1750, type: 'guard' },
      { x: 2050, type: 'guard' }, { x: 2400, type: 'guard' },
      { x: 2650, type: 'guard' }, { x: 3000, type: 'guard' },
      { x: 3300, type: 'guard' }, { x: 3550, type: 'guard' },
      { x: 3850, type: 'guard' }, { x: 4050, type: 'guard' },
      { x: 4350, type: 'guard' }, { x: 4550, type: 'guard' },
      { x: 4850, type: 'guard' }, { x: 5050, type: 'guard' },
      { x: 5350, type: 'guard' }, { x: 5550, type: 'guard' },
      { x: 5750, type: 'guard' },
      // Shooters
    ...
    for (const ed of enemyDefs) {
      const e = new Enemy(ed.x, ed.y || 0, ed.type);
      if (ed.type !== 'drone') {
        for (const p of platforms) {
  ...
  update(dt) {
    ...
    for (let bi = bullets.length - 1; bi >= 0; bi--) {
      ...
      for (const e of this.enemies) {
        ...
        if (b.x > e.x && b.x < e.x + e.w && b.y > e.y && b.y < e.y + e.h) {
          if (b.weapon && b.weapon.name === 'LASER') continue;
          b.alive = false;
    ...
    // Clean up dead bullets and particles
  ...
  draw() {
    ...
    // Stars (static, computed once per frame, parallax)
    ...
    // Mountains (parallax) - optimized
    this.drawMountains();

    // Jungle background - optimized with viewport clipping
    this.drawJungle();
  ...
  drawMountains() {
    ...
    for (let x = 0; x <= GAME_WIDTH; x += 40) {
      ...
      if (x === 0) ctx.moveTo(x, GAME_HEIGHT);
      ctx.lineTo(x, GAME_HEIGHT - h);
    }
    ctx.lineTo(GAME_WIDTH, GAME_HEIGHT);
    ctx.fill();
  ...
  drawJungle() {
    // Only draw trees that are within or near the viewport
    const viewLeft = camera.x - 50;
    const viewRight = camera.x + GAME_WIDTH + 50;

    ctx.fillStyle = '#0a2a0a';
    for (let wx = Math.floor(viewLeft / 30) * 30; wx < viewRight; wx += 30) {
      const h = Math.sin(wx * 0.02) * 15 + 30 + Math.sin(wx * 0.05) * 10;
      const sx = wx - camera.x;
      ctx.fillRect(sx, GAME_HEIGHT - h, 20, h);
      ctx.fillStyle = '#0a3a0a';
      ctx.beginPath();
      ctx.arc(sx + 10, GAME_HEIGHT - h, 15 + Math.sin(wx) * 5, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = '#0a2a0a';
    }
  },
